...
@click.option('--output_filename', default='output.csv', help='The output file to store the extracted information.')
@click.option('--output_format', default='CSV', type=click.Choice(['CSV']), help='The output format for the extracted information.')
def main(image_folder: str, output_filename: str, output_format: str):
    """
    ...
    Args:
        ...
        output_filename (str): Path to the output file. Defaults to 'output.csv'.
        output_format (str): Format of the output file. Can be 'CSV' or 'TABLE'. Defaults to 'CSV'.

    Example usage:
        $ python cli.py /path/to/images --output_filename=books_info.csv --output_format=CSV
    """
    ...
    for book_id, book_image_paths in books.items():
        ...
        result: DataFrame = process_images(repo, book_images, 'Antropic - Claude Sonnet 3.5', output_format)
        all_results.append(result)

    df = concat(all_results, ignore_index=True)
    repo.get_output_generator(output_format).save(df, output_filename)  # TODO Implement

    print(f"Extracted information for {len(books)} books written to {output_filename}")
